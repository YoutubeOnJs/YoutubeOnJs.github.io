...
function bindSlideToggl(trigger, boxBody, content, openClass) { 
    let button = {
    ...
    button.element.addEventListener('click', () => {
        if (button.active === false) { // Проверяем меню на неактивность
            button.active = true; // Если она не активна - делаем ее активной
...
bindSlideToggl('.hamburger', '[data-slide="nav"]', '.header__menu', 'slide-active');
...
function switchMode() { 
    if (night === false){
        ...
        document.querySelectorAll('.videos_item-views').forEach(item => {
            item.style.color = '#fff';
        });
...
function start() { 
    gapi.client.init({
        'apiKey': 'AIzaSyAMDoocOI3tadDHM-HeqIoZBjK-wAii8TQ',
...
document.onkeydown = function (e) { // Для закрытия модальных окон на esc
    if (e.keyCode == 27) {
        closeModal();
        player.stopVideo();
    }
